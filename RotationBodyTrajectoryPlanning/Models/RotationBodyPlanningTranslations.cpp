...
namespace smrobot::workbench::spray::rotationbody
{
    namespace
    {
        ...
        constexpr TranslationEntry entries[] = {
            ...
            { "trajectory.spray_distance", "Base -Y spray distance", "基坐标 -Y 喷涂距离" },
            ...
            { "trajectory.auto_two", "Auto Generate 2", "自动生成两道轨迹" },
            { "trajectory.auto_two_tooltip", "Append two full-boundary trajectories using weighted upper/lower wall, tooth-top and tooth-bottom normals.", "根据上/下齿壁、齿顶和齿底的加权法向角，向轨迹组追加两道完整边界轨迹。" },
            { "trajectory.auto_three", "Auto Generate 3", "自动生成三道轨迹" },
            { "trajectory.auto_three_tooltip", "Append upper-wall/tooth-top, lower-wall/tooth-top and tooth-bottom trajectories.", "向轨迹组追加上齿壁/齿顶、下齿壁/齿顶和齿底三道轨迹。" },
            { "trajectory.import_parameters", "Append Trajectories from TXT", "从 TXT 追加轨迹" },
            { "trajectory.import_parameters_tooltip", "Read six numeric values per trajectory: distance, tilt, speed, start extension, end extension and positioner rpm.", "每道轨迹读取六个数值：喷涂距离、倾角、速度、上延长、下延长和转台转速。" },
            { "trajectory.import_dialog", "Choose Trajectory Parameter TXT", "选择轨迹参数 TXT" },
            { "trajectory.import_filter", "Trajectory parameters (*.txt);;All files (*.*)", "轨迹参数 (*.txt);;所有文件 (*.*)" },
            ...
            { "trajectory.cycle_count", "Trajectory group spray cycles", "循环喷涂轨迹组遍数" },
            { "trajectory.transition_between_cycles", "Interval between group cycles", "轨迹组循环间隔" },
            { "trajectory.transition_after", "Interval to next trajectory", "到下一条轨迹的时间间隔" },
            { "trajectory.transition_between", "Interval: trajectory %1 to trajectory %2", "轨迹 %1 到轨迹 %2 的时间间隔" },
            { "trajectory.transition_between_pairs", "Interval: trajectory %1 return to trajectory %2 forward", "第 %1 道轨迹返程到第 %2 道轨迹去程的时间间隔" },
            ...
            { "model.automatic_alignment", "Automatically align model", "自动摆正模型" },
            { "model.automatic_alignment_tooltip", "When selected, align the complete part to the planning frame. Clear it to retain the source model coordinates.", "勾选后按规划坐标系自动摆正完整件；取消勾选后保留模型文件的原始坐标。" },
            ...
            { "frame.confirmation_title", "Workpiece Frame", "工件坐标系" },
